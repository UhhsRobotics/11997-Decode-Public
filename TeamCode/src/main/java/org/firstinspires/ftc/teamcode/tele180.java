...
@Photon
@TeleOp(name = "180TEle", group = "TeleOp")
public class tele180 extends LinearOpMode {
    @Override
    public void runOpMode() throws InterruptedException {
        Hardware hardware = Drive.Globals.hardwareNames;
        ServoGroup servos = Drive.initServos(hardwareMap);
        ...
        while (opModeIsActive()) {
            ...
            if (Math.abs(X) > 0.05 || Math.abs(Y) > 0.05) {
                ...
                double Right = ((power - r) / max)*signum;
                double Left = ((power + r) / max)*signum;
